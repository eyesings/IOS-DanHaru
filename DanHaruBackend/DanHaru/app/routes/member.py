...
from app.model.mem_models import MemberRegister, MemberToken, MemberMe, MemberLogin, ValidationType, MemberValidation, MemberReturn, SnsType
...
PATHDIR = "/Users/radcns_kim_taewon/TaeWOn/IOS-DanHaru/DanHaruBackend/DanHaru/profile/"
...
async def mem_register(sns_type: SnsType, reg_info: MemberRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email:
        ...
        new_user = Member.create(session, auto_commit=True, mem_id=reg_info.id, mem_pw=hash_pw, mem_email=reg_info.email, created_user=reg_info.id, updated_user=reg_info.id)
        token = dict(Authorization=f"Bearer {create_access_token(data=MemberToken.from_orm(new_user).dict(exclude={'pw', 'mem_id'}),)}")
        print("token::", token)
