...
class FMCropView: UIView {
    ...
    //default crop 1:1
    public var defaultCrop: Bool = false
    public var changeRatio: Bool = false
    ...
    private var stickerFrame: CGRect = .zero
    ...
    private func moveCroppedContentToCenterAnimated(complete: (() -> Void)? = nil) {
        ...
        if cropRatio == nil {
            if changeRatio {
                cropRatio = FMCropRatio(width: 1.0, height: 1.0)
            }
        }
        ...
        if cropRatio == nil && !self.defaultCrop { //오리지널 크롭에서 필터, 스티커 탭으로 갔다가 자르기 탭으로 와도 현재 탭 유지
            let ratio = image.size.height / image.size.width
            
            // correct ratio only
            cropFrame.size.height = cropFrame.size.width * ratio
        } else if cropRatio == nil && self.defaultCrop {    // Ratio가 nil 로 들어오면 1:1   // 에디터에서 자동으로 1:1 자르기 영역으로
            self.defaultCrop = false
            let ratio = CGFloat(1.0) / CGFloat(1.0)
        ...
        UIView.animate(withDuration: kComplexAnimationDuration,
                       ...
                       animations: {
                        ...
                        self.setViewFrmae(frame: cropFrame)
        },
                       completion: { _ in
    ...
    private func cropboxViewFrameDidChange(rect: CGRect) {
        foregroundView.frame = rect     // -> crop메뉴 선택 시 backgournd blur처리
    ...
    // crop된 이미지 frame 세팅
    private func setViewFrmae(frame: CGRect) {
        self.stickerFrame = frame
    }
    
    // crop된 이미지 frame 리턴
    public func getViewFrame() -> CGRect {
        return self.stickerFrame
    }
