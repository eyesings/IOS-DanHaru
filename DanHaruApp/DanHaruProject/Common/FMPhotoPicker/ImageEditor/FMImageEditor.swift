...
struct FMImageEditor {
    ...
    init(crop: FMCroppable=kDefaultCrop, cropArea: FMCropArea=FMCropArea()) {
    ...
    func reproduce(source image: UIImage, cropState: FMImageEditState) -> UIImage {
        var result = image
    ...
    public func isEdited() -> Bool {
        return crop.identifier() != initCrop.identifier() && !cropArea.isApproximatelyEqual(to: initCropArea)
    }
}
